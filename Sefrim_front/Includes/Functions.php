...
function TotalPromotion(){
  global $ConnectingDB;
  $sql = "SELECT * FROM product Where tauxpromo != 0 LIMIT 4";
  $stmt   = $ConnectingDB->prepare($sql);
  $stmt->execute();
}
...
function display_top4_promotions(){
  ...
  global $ConnectingDB;
  $sql = "SELECT * FROM product where tauxpromo != 0 ORDER BY tauxpromo desc LIMIT 4";
  ...
  while ($DataRows=$Execute->fetch()) {
    ...
    echo'
    <div class="grid12-3">
      <div><img src="images/'.$img.'" alt="custom-image"></div>
      <p>'.$name.'</p>
...
function display_all_promotions($max_promo,$type){
    global $ConnectingDB;
    $sql = "SELECT * FROM product where tauxpromo != 0 ORDER BY $type ASC LIMIT {$max_promo}";
    $Execute =$ConnectingDB->query($sql);
    ...
    while($DataRows=$Execute->fetch(PDO::FETCH_ASSOC)){
      ...
      <li class="item col-lg-4 col-md-3 col-sm-4 col-xs-6">
        <div class="item-inner">
          <div class="item-img">
            <div class="item-img-info"><a href="product-detail-promo.html" title="Retis lapen casen" class="product-image"><img src="images/'.$img.'" alt="Retis lapen casen"></a>
...
function display_list_promotions($max_promo,$type){
  ...
  while($DataRows=$Execute->fetch(PDO::FETCH_ASSOC)){
    ...
    echo'
    <li class="item first">
      <div class="product-image"> <a href="product-detail.html" title="HTC Rhyme Sense"> <img class="small-image" src="images/'.$img.'" alt="HTC Rhyme Sense"> </a> </div>
