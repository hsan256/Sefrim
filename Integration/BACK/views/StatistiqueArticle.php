...
$db=config::getConnexion();
  ...
  //shipping stats...
  $req3= $db->query("SELECT `Titre` , count(*) as nb_commentaire from `article` JOIN `commentaire` WHERE article.Id = commentaire.Id_Article GROUP BY `Titre`");
  $req3->execute();
...
foreach ($liste3 as $row) {
   $dataPoints3[] = array('y' => $row->nb_commentaire ,'label' => $row->Titre);
}
?>
...
var chart = new CanvasJS.Chart("chartContainer", {
  ...
  title: {
    text: "Nombre de commentaire(s) Par Article"
  },
  data: [{
    type: "pie",
    startAngle: 240,
    yValueFormatString: "##0\" Commentaire(s) Par \"",
    indexLabel: "{y} {label}",
